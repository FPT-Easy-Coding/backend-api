...
public class QuizController {
    ...
    public ResponseEntity<MessageResponse> updateQuiz(
            @Valid
            @RequestBody UpdateQuizRequest quizRequest) {
        try {
            Quiz quiz = quizServiceImpl.updateQuiz(quizRequest);
            if (quiz == null) {
                return ResponseEntity.ok(
                        MessageResponse.builder()
                                .success(false)
                                .msg("Quiz with given ID not found")
                                .build()
                );
            } else {
                return ResponseEntity.ok(
